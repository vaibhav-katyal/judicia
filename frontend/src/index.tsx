...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Judicia — Intelligent Legal Research & Case Retrieval" },
      {
        name: "description",
        content:
          "Judicia retrieves the statutes, judgments and articles that answer your legal question, with citations and relevance context.",
      },
      { property: "og:title", content: "Judicia — Intelligent Legal Research" },
      {
        property: "og:description",
        content:
          "Ask a legal question in plain language. Judicia surfaces the governing provisions and judgments with citations.",
      },
    ],
  }),
  component: Judicia,
...
function IdleState() {
  const pillars = [
    {
      k: "01",
      t: "Grounded in the source",
      d: "Every answer points back to a provision or reported judgment — never an unsourced summary.",
    },
    {
      k: "02",
      t: "Passage-level retrieval",
      d: "Judicia surfaces the operative paragraph, not the whole volume, so you read what matters.",
    },
    {
      k: "03",
      t: "Explained relevance",
      d: "Each result states why it matched, so you can accept or discard it in seconds.",
    },
  ];
  return (
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {result.passages.map((p) => (
          <span
            key={p}
            className="rounded-full bg-secondary px-2.5 py-1 text-xs text-ink-soft"
          >
            {p}
          </span>
        ))}
